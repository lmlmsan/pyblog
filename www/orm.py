...
async def execute(sql, args, autocommit=True):
    log(sql)
    print("sql.replace('?', '%s') is :", sql.replace('?', '%s'))
...
class StringField(Field):
	...
	def __init__(self, name=None, primary_key=False,default=None,ddl='vachar(100)'):
	...
	def __new__(cls, name, bases, attrs):
		...
		for k, v in attrs.items():
			if isinstance(v,Field):
				...
				mappings[k] = v
				if v.primary_key:
					print("v.primary_key is:",v.primary_key)
					# 找到主键：
					if primaryKey:
		...
		print("***primaryKey is: ",primaryKey)
...
class Model(dict, metaclass=ModelMetaclass):
	...
	async def save(self):
		...
		print("****self.__insert__ is : ",self.__insert__)
		print("****args is :", args)
